...
class MuxSelectorNode():
    def __init__(self):
        ...
        self.ext_nav_topic = RosParam.get_param(self, 'autopilot_ros_config.twist_mux_topics.ext_nav') #/nav_cmd_vel 
        self.key_teleop_topic = RosParam.get_param(self, 'autopilot_ros_config.twist_mux_topics.key_teleop') #/key/cmd_vel
        self.sp_vel_topic = RosParam.get_param(self, 'autopilot_ros_config.twist_mux_topics.sp_vel') #/sp_vel/cmd_vel
        BaseNode.get_logger(self).info(str(self.ext_nav_topic))
        ...
        self.ext_nav_publisher = BaseNode.Publisher(self, Twist, self.ext_nav_topic, 10)
        self.key_teleop_publisher = BaseNode.Publisher(self, Twist, self.key_teleop_topic, 10)
        self.sp_vel_publisher = BaseNode.Publisher(self, Twist, self.sp_vel_topic, 10)
        self.twist_out = Twist()

    def mux_publisher(self):
        twist_out = Twist()

        if RosParam.get_param(self,parameter_name='mux_select') == self.ext_nav_topic : #/nav_cmd_vel 
            ...
            self.ext_nav_publisher.publish(twist_out)
            BaseNode.get_logger(self).info("mux_selector using ext_nav velocity.")

        elif RosParam.get_param(self,parameter_name='mux_select') == self.key_teleop_topic: #/key/cmd_vel
            ...
            self.key_teleop_publisher.publish(twist_out)
            BaseNode.get_logger(self).info("mux_selector using key_teleop velocity.")

        elif RosParam.get_param(self,parameter_name='mux_select') == self.sp_vel_topic: #/sp_vel/cmd_vel
            ...
            self.sp_vel_publisher.publish(twist_out)
