...
class gazebo::ArduPilotPluginPrivate
{
  ...
  public: rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr _demo_pub;
  public: geometry_msgs::msg::Twist _twist_data;
...
void ArduPilotPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  // const gazebo_ros::QoS & qos = dataPtr->ros_node_->get_qos();

  dataPtr->_demo_pub = dataPtr->ros_node_->create_publisher<geometry_msgs::msg::Twist>("apm_out", 10);
...
void ArduPilotPlugin::ApplyMotorForces(const double _dt)
{
  ...
  geometry_msgs::msg::Twist _twist_data;
  ...
  for (size_t i = 0; i < this->dataPtr->controls.size(); ++i)
  {
    ...
    else
    {
      ...
      _twist_data.linear.y = twist_values[0];
      _twist_data.linear.z = twist_values[1];
      _twist_data.angular.x = twist_values[2];
      _twist_data.angular.y = twist_values[3];

      _twist_data.linear.x = (twist_values[0]+twist_values[1]+twist_values[2]+twist_values[3])/2; //scale down linear vel
      _twist_data.angular.z = (-twist_values[0]+twist_values[1]-twist_values[2]+twist_values[3]);
      dataPtr->_demo_pub->publish(_twist_data);
  }
}
